...
const App = (() => {
    ...
    function renderSessions() {
        ...
        sessionsGrid.innerHTML = questionsData.sessions.map((session, idx) => {
            const isLocked = !Auth.isLoggedIn() && idx > 0;
            const totalQuestions = Object.values(session.levels)
    ...
    async function renderLevels(sessionId) {
        const session = questionsData.sessions.find(s => s.id === sessionId);
        if (!session) return;
        ...
        levelsGrid.querySelectorAll('.level-card:not(.locked)').forEach(card => {
            card.addEventListener('click', () => {
                Quiz.start(card.dataset.session, card.dataset.level);
                showScreen('quiz');
            });
        });
    ...
    function bindEvents() {
        ...
        retryBtn.addEventListener('click', () => {
            Quiz.start(Quiz.getCurrentSession().id, Quiz.getCurrentLevel());
            showScreen('quiz');
        });
    ...
    async function init() {
        ...
        questionsData = await Quiz.loadQuestions();
        renderSessions();
